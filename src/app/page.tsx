...
export default function Home() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Aegis Dashboard</h1>
        <p className="text-sm text-gray-500 mt-1">
          Privacy agent on Base — delegated spending via MetaMask Delegation Framework
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-aegis-card border border-aegis-border rounded-xl p-6">
          <div className="flex items-center gap-2 mb-3">
            <svg className="w-5 h-5 text-aegis-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {error ? (
            ...
            <div className="space-y-2 text-sm font-mono">
              <div>
                Setup:{" "}
                <span className={ready ? "text-aegis-green" : "text-yellow-500"}>
                  {status.setup}
                </span>
              </div>
        ...
        <div className="bg-aegis-card border border-aegis-border rounded-xl p-6">
          ...
          {status ? (
            <div className="space-y-2 text-sm">
              <div className="p-3 bg-aegis-bg rounded-lg border border-aegis-border">
                <span className="text-aegis-accent font-mono text-lg">{status.spendingPolicy}</span>
              </div>
      ...
      <div className="bg-aegis-card border border-aegis-border rounded-xl p-6">
        ...
        <form onSubmit={runCommand} className="flex gap-3">
          <input
            ...
            className="flex-1 bg-aegis-bg border border-aegis-border rounded-lg px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-aegis-accent"
          />
          <button
            type="submit"
            disabled={running}
            className="bg-aegis-accent hover:bg-indigo-600 disabled:opacity-50 px-5 py-2.5 rounded-lg text-sm font-medium text-white transition"
          >
        ...
        {output && (
          <pre className="mt-4 p-4 bg-aegis-bg border border-aegis-border rounded-lg text-sm text-green-400 font-mono whitespace-pre-wrap overflow-x-auto max-h-64 overflow-y-auto">
            {output}
          </pre>
        )}
      </div>
      ...
      {/* How it works */}
      <div className="bg-aegis-card border border-aegis-border rounded-xl p-6">
        <h3 className="text-sm font-medium text-gray-400 mb-3">How Aegis Keeps Secrets</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <div className="p-3 bg-aegis-bg rounded-lg">
            <p className="text-aegis-accent font-medium mb-1">1. Delegated Authority</p>
          ...
          <div className="p-3 bg-aegis-bg rounded-lg">
            <p className="text-aegis-accent font-medium mb-1">2. Private Reasoning</p>
            <p className="text-gray-500">
          ...
          <div className="p-3 bg-aegis-bg rounded-lg">
            <p className="text-aegis-accent font-medium mb-1">3. Identity Separation</p>
            <p className="text-gray-500">
